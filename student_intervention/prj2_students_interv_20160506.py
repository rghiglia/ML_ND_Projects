...
# Reviewing this problem and extending the number of classifiers
...
# Read/load the data
# -----------------------------------------------------------------------------
data_org = pd.read_csv(r"C:\Users\rghiglia\Documents\ML_ND\student_intervention\student-data.csv")
type(data_org)  # dataframe
print "Data loaded"
...
from rg_toolbox_data import preproc_data, cat2num
data_num_org = preproc_data(data_org,5)
data = cat2num(data_num_org) # force to num
data.info()

# Decide the features to keep or the ones to toss
col_drop = ['age', 'absences'];
data = data.drop(col_drop, axis=1)


# At this stage all data should be numeric (both inputs and outputs)

# There are three tasks then: separating features from target, reshuffling, and splitting into train and test
# It seems you should keep them together as much as possible, so:
# 1. Reshuffle
# 2. Split features from target
# 3. Split train and test
# Unfortunately some of the tasks are embedded in the routines in sklearn
# Shuffle and split are done together, so I'll start with features and target
# From now onwards I will use this convention:
# X_trn: nT x nF DataFrame of training data
# y_trn: nT x 1 Series of training data
# X_tst: nT x nF DataFrame of testing data
# y_tst: nT x 1 Series of testing data

# Extract target
col_tgt = 'passed'
X = data.drop(col_tgt, axis=1)
y = data[col_tgt]

# Define size of training set
pct_data = 0.75
nO, nF = X.shape
nTrn = int(float(nO)*pct_data)
nTst = nO - nTrn
if nTrn+nTst!=nO: print "Error: training and test don't sum up"
yU = sorted(set(y))
is_cls = len(yU)==2
...
print "Classification problem:  '%s'\n" % is_cls
if is_cls:
    ix0 = y[y==yU[0]].index
    ix1 = y[y==yU[1]].index
    print "%s = %i \t\t %1.1f%%" % (col_tgt, yU[0], float(len(ix0))/nO*100)
    print "%s = %i \t\t %1.1f%%" % (col_tgt, yU[1], float(len(ix1))/nO*100)
...
X_trn, y_trn = X.loc[ix_trn], y[ix_trn]
X_tst, y_tst = X.loc[ix_tst], y[ix_tst]


# Sets for CV
pctCV_tst = 0.15 # CV test
nCV_tst = int(float(nTrn)*pctCV_tst)
...
clf_NB_ = GaussianNB()
out_trn_NB_ = train_clf(clf_NB_, X_CV_trn, y_CV_trn);
out_prd_NB_ = pred_clf(clf_NB_, X_CV_trn, y_CV_trn)
print "Prediction with %s F1 = %1.4f" % (clf_NB_.__class__.__name__ ,out_prd_NB_['score'])

# Logistic Regression
from sklearn.linear_model import LogisticRegression
clf_LR_ = LogisticRegression()
out_trn_LR_ = train_clf(clf_LR_, X_CV_trn, y_CV_trn);
out_prd_LR_ = pred_clf(clf_LR_, X_CV_trn, y_CV_trn)
print "Prediction with %s F1 = %1.4f" % (clf_LR_.__class__.__name__ ,out_prd_LR_['score'])
...
out_trn_kNN = train_clf(clf_kNN, X_CV_trn, y_CV_trn);
out_prd_kNN = pred_clf(clf_kNN, X_CV_trn, y_CV_trn)
print "Prediction with %s F1 = %1.4f" % (clf_kNN.__class__.__name__ ,out_prd_kNN['score'])

## ANN
#from sklearn.neural_network import MLPClassifier
#clf_ANN = MLPClassifier()
#out_trn_ANN = train_clf(clf_ANN, X_CV_trn, y_CV_trn);
#out_prd_ANN = pred_clf(clf_ANN, X_CV_trn, y_CV_trn)
#print "Prediction with %s F1 = %1.4f" % (clf_ANN.__class__.__name__ ,out_prd_ANN['score'])

# SVM
from sklearn import svm
clf_SVM = svm.SVC(C=1.0)
out_trn_SVM = train_clf(clf_SVM, X_CV_trn, y_CV_trn);
out_prd_SVM = pred_clf(clf_SVM, X_CV_trn, y_CV_trn)
print "Prediction with %s F1 = %1.4f" % (clf_SVM.__class__.__name__ ,out_prd_SVM['score'])


# Next step: encapsulate
out_NB_ = run_clf('NB', X_CV_trn, y_CV_trn, X_CV_tst, y_CV_tst);
out_LR_ = run_clf('LR', X_CV_trn, y_CV_trn, X_CV_tst, y_CV_tst);
out_DT_ = run_clf('DT', X_CV_trn, y_CV_trn, X_CV_tst, y_CV_tst);
out_kNN = run_clf('kNN', X_CV_trn, y_CV_trn, X_CV_tst, y_CV_tst);
out_SVM = run_clf('SVM', X_CV_trn, y_CV_trn, X_CV_tst, y_CV_tst);

score_ins = {'NB': out_NB_['out_prd_ins']['score'], 
 'LR': out_LR_['out_prd_ins']['score'], 
 'DT': out_DT_['out_prd_ins']['score'], 
 'kNN': out_kNN['out_prd_ins']['score'],
 'SVM': out_SVM['out_prd_ins']['score']}

score_ots = {'NB': out_NB_['out_prd_ots']['score'], 
 'LR': out_LR_['out_prd_ots']['score'], 
 'DT': out_DT_['out_prd_ots']['score'], 
 'kNN': out_kNN['out_prd_ots']['score'],
 'SVM': out_SVM['out_prd_ots']['score']}

# Create a DF
x1 = Series(score_ins)
x2 = Series(score_ots)
df = pd.concat([x1, x2], axis=1)
df.columns = ['In-sample', 'Out-of-sample']
df = df.reindex(['NB', 'LR', 'DT', 'kNN', 'SVM'])
...
clf_RF_ = RandomForestClassifier(n_estimators=10)
out_trn_RF_ = train_clf(clf_RF_, X_CV_trn, y_CV_trn);
out_prd_RF_ = pred_clf(clf_RF_, X_CV_trn, y_CV_trn)
print "Prediction F1 = %1.4f" % out_prd_RF_['score']

# Gradient Boosting
from sklearn.ensemble import GradientBoostingClassifier
clf_GB_ = GradientBoostingClassifier(n_estimators=10)
out_trn_GB_ = train_clf(clf_GB_, X_CV_trn, y_CV_trn);
out_prd_GB_ = pred_clf(clf_GB_, X_CV_trn, y_CV_trn)
print "Prediction F1 = %1.4f" % out_prd_GB_['score']
...
print "Summary Paramaters of Classifiers"
display(df_descr)
...
# Try to plot the boundary:
# Choose two continuous variables, probably absences and either age or another important feature
# Define a grid of points in twsoe 2 dimensions
# Fix all other variables to the the most common entry
# If variables are continuous you can set it to the median
    ...
    if df[col].dtype != object:
        ...
        if len(u)>2:
            ...
            if len(df[col]==u[0]) > len(df[col]==u[1]):
                ...
                df[col] = u[1]
X_train_avg = df
# Ok but you really need a repmat(df.ix[0,:],xx.shape[0],1)
...
clf = svm.SVC()
clf.fit(X_train, y_train)
clf.fit(X_train.values, y_train_bol)
y_tmp = clf.predict(X_train_avg)
# np.r_ I think is the same as X(:) in Matlab, np.c_ does concatenation along axis=1
# xx.ravel = "X'(:)" in Matlab
# [xx.ravel(), yy.ravel()] is a 2-list of vectorized values
# Unexpected role of np.c_: it created a DataFrame with the two vectorized lists as colmuns ... go figure
...
#pl.contourf(xx, yy, y_tmp, cmap=pl.cm.Paired, alpha=0.8)
pl.contourf(xx, yy, y_tmp, alpha=0.8)

# Plot also the training points
#pl.scatter(X_train['absences'], X_train['age'], c=y, cmap=pl.cm.Paired)
ixY = y_tgt==1
ixN = y_tgt==0
...
Facs = np.zeros((10,5))
Facs = DataFrame(Facs, columns=('1', '2', '3', '4', '5'))
for i in range(10):
    ...
    zs = z.order(ascending=False)
    ixsDT = zs.index
    Facs.ix[i,:] = ixsDT[:5].values
    
Facs.index = 1+np.arange(10)

# Most popular
##facs = Series(Facs.values.ravel())
#facs = Facs.values.ravel()
#type(facs)  # numpy.ndarray

#import collections
#fac_hist = collections.Counter(facs)
#type(fac_hist) # collections.Counter
#
#facs = DataFrame(Facs.values.ravel())
#fac_df = dict((x, facs.count(x)) for x in facs)

from collections import Counter
facs = Facs.values.ravel()
type(facs)  # numpy.ndarray
fac_counts = Counter(facs)
df = DataFrame.from_dict(fac_counts, orient='index')
#df.columns = 'Fac' # should work ... :(
#df.columns(0) = 'Fac'
df.rename(columns = {0:'Fac'}, inplace = True) # works
df.sort('Fac', ascending=False, inplace=True)
df.plot(kind='bar')
fac_tmp = df.loc[df.Fac>1]
type(fac_tmp) # DataFrame
fac_tmp = Series(df.loc[df.Fac>1])
type(fac_tmp) # Series
#fac_tmp = tuple(df.loc[df.Fac>1])
#type(fac_tmp) # tuple

X_train_compressed = X_train[fac_tmp.index]
X_test_compressed = X_test[fac_tmp.index]
...
clf = svm.SVC()
clf_gs = GridSearchCV(clf,prm,scoring=sc_fnc,cv=3)
clf_gs.fit(X_train_compressed,y_train)
print clf_gs.best_estimator_
clf_gs.predict(X_test_compressed)

f1_score(y_test.values, clf_gs.predict(X_test_compressed), pos_label='yes')
# Not a lot better
...
f1_score(y_test.values, clf.predict(X_test_compressed), pos_label='yes')
...
# Estimating confidence interval of classifier
# Conceptually: do a cross-validation thing on a given algo
# Measure performance scsore on both training and testing set
