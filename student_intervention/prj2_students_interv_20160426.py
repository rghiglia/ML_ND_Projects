...
#type(X_tmp_aug)
#type(y_tmp)
## df, Series (of yes/no values)
#X_tmp_aug, y_tmp = shuffle(X_tmp_aug, y_tmp, random_state=1)
#type(X_tmp_aug)
#type(y_tmp)
## np.ndarray, np.ndarray (of yes/no values)
## It damn changes the type!

ixs = shuffle(X_tmp_aug.index, random_state=0)
X_tmp_aug, y_tmp, y_tpp = X_tmp_aug.reindex(ixs), y_tmp.reindex(ixs), y.reindex(ixs)

X_train, y_train, y_train_bol = X_tmp_aug.ix[:num_train,:], y_tmp[:num_train], y_tpp[:num_train]
X_test, y_test, y_test_bol = X_tmp_aug.ix[num_train:,:], y_tmp[num_train:], y_tpp[num_train:]
