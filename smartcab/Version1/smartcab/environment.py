...
#            agent.reset(destination=(destination if agent is self.primary_agent else None))
    ...
    def reset(self, stt=(-1, -1), dst=(-1, -1)):
        self.done = False
        self.t = 0
...
#        print "Resetting of e being called"
        ...
        print "Env: Start {}, Destination {}".format(start, destination)
...
#                #self.loc = self.primary_agent['location']
#                print agent
#                print(dir(agent))
    ...
    def act(self, agent, action):
        ...
        state = self.agent_states[agent]
        location = state['location']    # location before move
        heading = state['heading']      # heading before move
        light = 'green' if (self.intersections[location].state and heading[1] != 0) or ((not self.intersections[location].state) and heading[0] != 0) else 'red'
        
        if agent is self.primary_agent:
            self.loc.append(location) #RG: we are appending the previous location; we will update the value of that location further down the code
#            print 'Loc', self.loc
        ...
        # We calculate the reward given heading which is a function of the action
        if action is not None:
            if move_okay:
                ...
                state['location'] = location    # update location
                state['heading'] = heading      # update heading
