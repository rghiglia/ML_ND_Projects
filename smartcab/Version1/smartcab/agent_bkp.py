    ...
    for sgy in sgys:
        ...
        for iS in range(len(L4)):
            ...
            for z in L4[iS][sgy][0]:
                ...
                y.append(z[1])
            plt.plot(x, y)
        plt.plot(stt0[0], stt0[1], 'k', marker='o', ms=10, markerfacecolor=None, label='Start')
        plt.plot(dst0[0], dst0[1], 'k', marker='s', ms=10, markerfacecolor=None, label='Stop' )
        plt.title(sgy)
        plt.xticks(range(10))
        plt.xticks(range(8))
        plt.xlim((0, 9))
        plt.ylim((0, 7))
        plt.savefig(r'C:\Users\rghiglia\Documents\ML_ND\ML_ND_Projects\smartcab\Path_' + sgy + '.png', bbox_inches='tight')
    
    ax = []
    for (k, sgy) in enumerate(sgys):
        fig = plt.figure(figsize=(8,8))
        ax.append(fig.add_subplot(2,2,k+1))
        for iS in range(len(L4)):
            (ax[-1]).plot(R4[iS][sgy][0]['cum_rwd'])
        ax_tmp = ax[-1]
        ax_tmp.set_title(sgy)
